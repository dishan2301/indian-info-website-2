...
export function contentSecurityPolicy(nonce, production = process.env.NODE_ENV === 'production') {
  return [
    "default-src 'self'",
    "base-uri 'self'",
    "connect-src 'self' https://www.google-analytics.com https://region1.google-analytics.com https://www.facebook.com https://formsubmit.co https://maps.googleapis.com https://maps.gstatic.com",
    "font-src 'self'",
    "form-action 'self'",
    "frame-ancestors 'none'",
    "frame-src https://www.google.com",
    "img-src 'self' data: blob: https://www.facebook.com https://maps.gstatic.com https://*.googleusercontent.com",
...
export function createEnquiryMailto(values) {
  ...
  const recipient = ['sales', 'indianinfotech.org'].join('@');
  const href = `mailto:${recipient}?subject=${encodeURIComponent('Website solution enquiry')}&body=${encodeURIComponent(body)}`;
...
export function validateContactSubmission(values) {
  const data = Object.fromEntries(Object.entries(CONTACT_LIMITS).map(([field, limit]) => [field, sanitizeText(values?.[field], limit)]));
  if (data.website) return { valid: true, spam: true, data, errors: [] };

  const errors = [];
  if (data.name.length < 2) errors.push('Please enter your name.');
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/u.test(data.email)) errors.push('Please enter a valid email address.');
  if (!/^[+0-9() .-]{7,40}$/u.test(data.phone)) errors.push('Please enter a valid phone number.');
  if (data.message.length < 10) errors.push('Please enter a message of at least 10 characters.');

  return { valid: errors.length === 0, spam: false, data, errors };
}
