...
export default async function SoftwareDetailPage({ params }: SoftwarePageProps) {
  ...
  return (
    <main>
      ...
      {software.publishedFacts?.length ? <section className="section software-published-specs"><div className="section-heading split-heading"><div><p className="section-kicker">Software scope</p><h2>Review configuration details before rollout.</h2></div><p>{software.evidenceSource}. These values describe the published system scope, not an automatic promise for every device, license, or deployment.</p></div><dl className="product-summary-list">{software.publishedFacts.map((fact) => <div key={fact.label}><dt>{fact.label}</dt><dd>{fact.value}</dd></div>)}</dl><div className="hero-actions"><a className="button button-primary" href={`/software/${software.slug}/specification`} download>Download specification summary ↓</a><Link className="button outline-button" href={`/contact?software=${software.slug}&resource=specification`}>Verify configuration</Link></div></section> : null}
