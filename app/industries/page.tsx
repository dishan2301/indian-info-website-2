...
export default function IndustriesPage() { return <main className="industries-page"><SiteHeader /><PageHero eyebrow="Industries" title="Different operating realities. One disciplined way to scope the system." description="Explore the workforce, entry, visitor, and workplace questions that shape a useful Indian Infotech recommendation." marker="II / INDUSTRIES" /><section className="section industries-directory-section"><div className="section-heading split-heading"><div><p className="section-kicker">Industry context</p><h2>Begin with how the site actually works.</h2></div><p>These pages are planning guides, not generic industry claims. Product fit, compliance context, and outcomes are confirmed with the customer.</p></div><RouteCardGrid cards={industryProfiles.map((item) => ({ title: item.name, description: item.context, href: `/industries/${item.slug}`, eyebrow: 'Operating context' }))} /></section><SiteFooter /></main>; }
