...
export function QuotesAndNews() {
  ...
  return <section className="home-quotes-news-page" aria-labelledby="client-quotes-heading">
    <div className="home-quote-heading-row"><Reveal className="home-section-heading"><p>Client’s Quote</p><h2 id="client-quotes-heading">Feedback from teams we support.</h2></Reveal><div className="home-quote-controls"><button type="button" onClick={() => setQuotePaused((current) => !current)} aria-label={quotePaused ? "Play client quotes" : "Pause client quotes"} aria-pressed={quotePaused}>{quotePaused ? "▶" : "Ⅱ"}</button><button type="button" onClick={() => moveQuote(-1)} aria-label="Previous client quote">←</button><button type="button" onClick={() => moveQuote(1)} aria-label="Next client quote">→</button></div></div>
    <div className="home-quote-viewport" role="region" aria-roledescription="carousel" aria-label="Client quotes"><div className="home-quote-track" style={{ transform: `translate3d(-${quoteActive * 100}%,0,0)` }}>{clientQuotes.map((item, index) => <blockquote aria-hidden={index !== quoteActive} className="home-quote-slide" key={item.company}><div><Image unoptimized className="home-quote-logo" src={item.logo} alt={`${item.company} logo`} width={120} height={42} /><b aria-hidden="true">“</b></div><p>{item.quote}</p><cite>— {item.company}</cite></blockquote>)}</div></div>
