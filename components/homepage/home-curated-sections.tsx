...
export function QuotesAndNews() {
  ...
  useEffect(() => { if (reducedMotion) return; const timer = window.setInterval(() => setQuoteActive((current) => (current + 1) % clientQuotes.length), 5500); return () => clearInterval(timer); }, [reducedMotion]);
