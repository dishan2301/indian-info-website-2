...
export function QuotesAndNews() {
  ...
  return <section className="home-quotes-news-page" aria-labelledby="client-quotes-heading">
    ...
    <div className="home-quote-grid">{clientQuotes.map((item) => <blockquote key={item.source}><div><span role="img" aria-label={`${item.mark} logo`}>{item.mark}</span><b aria-hidden="true">“</b></div><p>{item.quote}</p><cite>— {item.source}</cite></blockquote>)}</div>
