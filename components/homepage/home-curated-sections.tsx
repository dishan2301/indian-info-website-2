...
export function QuotesAndNews() {
  ...
  const [newsMotion, setNewsMotion] = useState(true);
  ...
  const moveQuote = (direction: number) => setQuoteActive((current) => direction > 0 ? (current >= clientQuotes.length - 1 ? clientQuotes.length : current + 1) : (current <= 0 ? clientQuotes.length - 1 : current - 1));
  const finishNewsLoop = () => { if (active !== news.length) return; setNewsMotion(false); setActive(0); requestAnimationFrame(() => requestAnimationFrame(() => setNewsMotion(true))); };
  const finishQuoteLoop = () => { if (quoteActive !== clientQuotes.length) return; setQuoteMotion(false); setQuoteActive(0); requestAnimationFrame(() => requestAnimationFrame(() => setQuoteMotion(true))); };
