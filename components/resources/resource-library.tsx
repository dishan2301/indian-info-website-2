...
const resources: readonly ResourceItem[] = [
  ...
  { title: 'AI technology in production lines', description: 'Legacy article listing dated 23 August 2024. The original record currently contains no article body and is under editorial review.', type: 'Archived topic', area: 'Company' },
  { title: 'Rent-based cloud attendance management', description: 'Legacy Easytime article listing dated 23 August 2024. The original record currently contains no article body and is under editorial review.', type: 'Archived topic', area: 'Software' },
] as const;
    ...
    <div className="resource-library">
      ...
      <div className="resource-grid">
        {filtered.map((resource) => {
          const content = <><span>{resource.type} · {resource.area}</span><h2>{resource.title}</h2><p>{resource.description}</p><b>{resource.href ? 'Open resource ↗' : 'Editorial review pending'}</b></>;
          return resource.href ? <Link className="resource-card" href={resource.href} key={resource.title}>{content}</Link> : <article className="resource-card archived" id={resource.title === 'AI technology in production lines' ? 'legacy-editorial' : undefined} key={resource.title}>{content}</article>;
        })}
      </div>
