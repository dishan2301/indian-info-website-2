...
export function ProductViewer({ name, images, frames = [] }: ProductViewerProps) {
  ...
  if (!total) return <div className="product-media-missing"><span>Media review</span><strong>Product imagery is being prepared.</strong><p>Request the current product reference from Indian Infotech.</p></div>;
  return <div className="product-viewer" role="region" aria-label={`${name} ${is360 ? '360 degree product explorer' : 'multi-angle product explorer'}`}>
    ...
    <div className="product-viewer-controls"><button type="button" onClick={() => move(-1)} aria-label={`Previous ${name} view`}>←</button><span>{is360 ? 'Drag, scrub, or use controls to rotate the frame sequence' : 'Swipe or use controls to inspect each product view'}</span><button type="button" onClick={() => move(1)} aria-label={`Next ${name} view`}>→</button></div>
    ...
    <p className="product-viewer-note"><strong>{is360 ? '360° sequence' : 'Multi-angle gallery'}</strong> {is360 ? 'This frame sequence is supplied for interactive product rotation.' : 'Use the available views to inspect the device; confirm the selected model, finish, and configuration before procurement.'}</p>
  </div>;
}
